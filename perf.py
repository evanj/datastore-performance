...
def benchmark_serialization(model_instance, response):
    ...
    output(response, 'serialized from model %d times in %f s\n' % (SERIALIZATION_ITERATIONS, end-start))
    ...
    output(response, 'serialized from datastore.Entity %d times in %f s\n' % (SERIALIZATION_ITERATIONS, end-start))

    start = time.time()
    for _ in xrange(SERIALIZATION_ITERATIONS):
        serialized = entity_proto.SerializeToString()
    end = time.time()
    output(response, 'serialized from entity_pb.EntityProto %d times in %f s\n' % (SERIALIZATION_ITERATIONS, end-start))
    ...
    output(response, 'deserialized to model %d times in %f s\n' % (SERIALIZATION_ITERATIONS, end-start))
    ...
    output(response, 'deserialized to datastore.Entity %d times in %f s\n' % (SERIALIZATION_ITERATIONS, end-start))

    start = time.time()
    for _ in xrange(SERIALIZATION_ITERATIONS):
        entity_proto = entity_pb.EntityProto(serialized)
    end = time.time()
    output(response, 'deserialized to entity_pb.EntityProto %d times in %f s\n' % (SERIALIZATION_ITERATIONS, end-start))
    ...
    output(response, '(entity has %d keys)\n' % len(entity))

    if deserialized.id != model_instance.id:
        output(response, 'WARNING: deserialized.id != input.id\n')
    ...
    output(response, 'protocol buffer property_size access in %f s\n' % (end-start))
    ...
    output(response, 'PythonListHolder property_size access in %f s\n' % (end-start))
